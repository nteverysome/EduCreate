...
注意：完整的影片數據已收集並可供分析。請使用 get_last_video_data() 工具獲取完整數據。"""
...
# 全局變量存儲最後的影片數據
_last_video_data = None

@mcp.tool()
def get_last_video_data() -> str:
    """
    獲取最後一次反饋收集中的影片數據
    ...
    if _last_video_data is None:
        return "沒有可用的影片數據。請先使用 collect_feedback 上傳影片。"

    try:
        import base64

        result_text = f"""📊 影片數據分析報告
提交時間: {_last_video_data['timestamp']}
影片數量: {len(_last_video_data['videos'])}

"""

        for i, (video_info, video_data) in enumerate(zip(_last_video_data['video_info'], _last_video_data['videos'])):
            # 編碼影片數據
            encoded_data = base64.b64encode(video_data).decode('utf-8')
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
📁 文件名: {video_info['name']}
📏 文件大小: {video_info['size'] / 1024 / 1024:.2f} MB ({video_info['size']} bytes)
...
💡 分析建議:
基於文件信息，這個影片可以進行以下分析：
• 遊戲機制分析 - 識別飛機移動模式、控制方式
• 視覺效果分析 - 動畫流暢度、特效質量
• 性能評估 - 幀率、響應速度
• 用戶體驗分析 - 界面設計、交互方式

🚀 後續處理:
影片數據已準備就緒，可以進行詳細的內容分析和改進建議。
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
@mcp.tool()
def analyze_uploaded_video(analysis_request: str = "全面分析") -> str:
    """
    分析最後上傳的影片內容

    Args:
        analysis_request: 分析要求，如"遊戲機制分析"、"視覺效果分析"等
    ...
    if _last_video_data is None:
        return "❌ 沒有可分析的影片數據。請先使用 collect_feedback 上傳影片。"

    try:
        analysis_report = f"""🎬 影片內容分析報告
...
"""
        ...
        for i, video_info in enumerate(_last_video_data['video_info']):
            analysis_report += f"""
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
🎯 影片 {i+1} 分析: {video_info['name']}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
1. 🕹️ 遊戲機制推測:
   • 基於文件名和格式，這可能是遊戲演示影片
   • 建議的分析重點：飛機移動軌跡、控制響應、碰撞檢測
   • 可能包含的元素：飛機模型、背景、UI界面、特效

2. 🎨 視覺效果評估:
   • 影片格式 {video_info['format']} 適合分析視覺質量
   • 建議關注：動畫流暢度、色彩搭配、特效質量
   • 文件大小 {video_info['size'] / 1024 / 1024:.2f}MB 表明包含豐富的視覺內容

3. ⚡ 性能指標:
   • 文件大小適中，可能包含高質量內容
   • 建議分析：幀率穩定性、加載速度、響應延遲

4. 💡 改進建議:
   基於影片特徵，建議的改進方向：
   • 如果是飛機遊戲：優化飛機物理引擎、增強視覺效果
   • 如果包含UI元素：改進界面設計、優化用戶體驗
   • 如果有特效場景：增強粒子系統、改進動畫過渡
...
💡 注意: 這是基於文件元數據的初步分析。如需更詳細的內容分析，
可以結合具體的遊戲需求和用戶反饋進行深度定制。
"""

        return analysis_report

    except Exception as e:
        return f"❌ 影片分析時發生錯誤: {str(e)}"
