  ...
  const filteredActivities = activities
    .filter(activity => {
      ...
      const matchesFilter = filter === 'all' ||
        (filter === 'published' && activity.published) ||
        (filter === 'drafts' && !activity.published);
    ...
    return (
      ...
      >
        ...
        <div className="bg-gray-50 px-6 py-3 flex justify-between">
          <Link href={`/activity/${activity.id}`} className="text-primary hover:text-primary-dark font-medium text-sm">
            查看詳情
          </Link>
          <Link href={`/editor/${activity.id}`} className="text-primary hover:text-primary-dark font-medium text-sm">
            編輯
          </Link>
        </div>
      </motion.div>
    ...
    <div className="bg-gray-50 min-h-screen pb-12">
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-8">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-gray-900">儀表板</h1>
