/**
 * 高級搜索系統 Playwright 端到端測試
 * 驗證六種搜索類型、八種排序選項、15個過濾器等功能
 */
...
test.describe('高級搜索系統', () => {
  ...
  test('應該能顯示高級搜索演示頁面', async ({ page }) => {
    ...
    // 驗證搜索類型選擇器
    await expect(page.locator('[data-testid="search-type-select"]')).toBeVisible();
  });
  ...
  test('應該能進行全文搜索', async ({ page }) => {
    ...
    await page.waitForResponse(response => 
      response.url().includes('/api/search/enhanced') &&
  ...
  test('應該能使用高級過濾器', async ({ page }) => {
    ...
    if (await resultItems.count() > 0) {
      ...
      // 檢查活動類型
      const activityType = firstResult.locator('[data-testid="activity-type"]');
      if (await activityType.isVisible()) {
        const typeText = await activityType.textContent();
        expect(typeText).toMatch(/matching|quiz/);
      }
      
      // 檢查GEPT等級
      const geptLevel = firstResult.locator('[data-testid="gept-level"]');
      if (await geptLevel.isVisible()) {
        await expect(geptLevel).toContainText('初級');
      }
    }
  });
  ...
  test('應該能設置日期範圍過濾器', async ({ page }) => {
    ...
    if (await resultItems.count() > 0) {
      const firstResult = resultItems.first();
      const dateElement = firstResult.locator('[data-testid="created-date"]');
  ...
  test('應該能切換排序選項', async ({ page }) => {
    ...
    for (const sortOption of sortOptions) {
      ...
      // 檢查排序選項是否正確選中
      const selectedValue = await page.locator('[data-testid="sort-by-select"]').inputValue();
      expect(selectedValue).toBe(sortOption.value);
    }
  });
  ...
  test('應該顯示搜索高亮', async ({ page }) => {
    ...
    if (await highlightElements.count() > 0) {
      ...
      // 驗證高亮樣式
      await expect(highlightElements.first()).toHaveCSS('background-color', /yellow|#ffff00/i);
    }
  });
  ...
  test('應該顯示分面過濾器', async ({ page }) => {
    ...
    // 驗證分面統計數字
    const facetItems = page.locator('[data-testid="facet-item"]');
    
    if (await facetItems.count() > 0) {
      const firstFacet = facetItems.first();
      await expect(firstFacet.locator('[data-testid="facet-count"]')).toBeVisible();
    }
  });
  ...
  test('應該能使用語義搜索', async ({ page }) => {
    // 輸入語義搜索查詢
    await page.fill('[data-testid="search-input"]', '學習英語的方法');
    ...
    // 驗證語義搜索結果
    await expect(page.locator('[data-testid="search-results"]')).toBeVisible();
    await expect(page.locator('[data-testid="search-stats"]')).toContainText('語義搜索');
  });

  test('應該能使用正則表達式搜索', async ({ page }) => {
    // 輸入正則表達式
    await page.fill('[data-testid="search-input"]', '英語.*學習');
    
    // 選擇正則表達式搜索類型
    await page.selectOption('[data-testid="search-type-select"]', 'regex');
