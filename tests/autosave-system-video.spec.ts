...
test.describe('完整自動保存系統 - 生成測試影片', () => {
  test('完整自動保存系統三層整合驗證', async ({ page }) => {
    ...
    await expect(page.locator('text=智能自動保存系統，確保您的工作永不丟失')).toBeVisible();
  ...
  test('完整自動保存系統功能驗證測試', async ({ page }) => {
    // 開始錄影
    await page.video();
    
    console.log('🎬 開始錄製完整自動保存系統功能驗證測試影片...');
    ...
    await expect(page.locator('text=安全加密')).toBeVisible();
    await expect(page.locator('text=性能優化')).toBeVisible();
    console.log('✅ 功能特性說明驗證成功');

    // 5. 驗證技術規範說明
    console.log('⚙️ 驗證技術規範說明');
    await expect(page.locator('text=零數據丟失保證')).toBeVisible();
    await expect(page.locator('text=智能自動保存系統')).toBeVisible();
    console.log('✅ 技術規範說明驗證成功');
