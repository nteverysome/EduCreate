...
console.log('🔴 最終正確修復 Mars 遊戲的 timeline 問題...');
...
// 創建一個正確的 timeline 替代函數
const timelineReplacement = `(function(config){
    if(config&&config.tweens){
        config.tweens.forEach((t,i)=>setTimeout(()=>this.scene.tweens.add(t),i*100));
        setTimeout(()=>config.onComplete&&config.onComplete(),config.tweens.length*100);
    }
    return{
        add:function(t){return this},
        play:function(){return this},
        destroy:function(){}
    };
}.bind(this))`;

// 替換 this.scene.tweens.timeline 為兼容函數
