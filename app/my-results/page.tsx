...
import WordwallStyleMyResults from '@/components/results/WordwallStyleMyResults';
  ...
  if (!currentUser) {
    return (
      <div className="min-h-screen bg-gray-50">
        ...
        <LoginPrompt
          title="登入以查看您的結果"
          description="請登入以查看您的課業分配結果和學生表現數據"
          redirectTo="/my-results"
        />
      </div>
    );
  }
          ...
          <div className="flex">
            ...
            <div className="ml-3">
              <p className="text-sm text-green-700">
                您正在使用演示模式。這是一個功能完整的演示，您可以查看課業分配結果。
              </p>
            </div>
          </div>
        </div>
      )}
      <WordwallStyleMyResults userId={currentUser.id} />
    </div>
  );
}
