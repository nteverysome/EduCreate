...
// 修復 Zenbaki 遊戲的資源路徑
function fixZenbakiPaths() {
    console.log('🔧 開始修復 Zenbaki 遊戲資源路徑...');
    
    const jsFilePath = 'public/games/zenbaki-game/dist/assets/index-BA24paxW.js';
    ...
    const pathReplacements = [
        // 圖片資源
        { from: /"assets\/images\/([^"]+)"/g, to: '"/games/zenbaki-game/dist/assets/images/$1"' },
        
        // 字體資源
        { from: /"assets\/fonts\/([^"]+)"/g, to: '"/games/zenbaki-game/dist/assets/fonts/$1"' },
        
        // 音效資源
        { from: /"assets\/sounds\/([^"]+)"/g, to: '"/games/zenbaki-game/dist/assets/sounds/$1"' },
        
        // CSS 資源
        { from: /"assets\/css\/([^"]+)"/g, to: '"/games/zenbaki-game/dist/assets/css/$1"' },
        
        // HTML 資源
        { from: /"assets\/html\/([^"]+)"/g, to: '"/games/zenbaki-game/dist/assets/html/$1"' },
        
        // 動態路徑修復 - 模板字符串和變數路徑
        { from: /`assets\/images\/cloud\$\{([^}]+)\}\.png`/g, to: '`/games/zenbaki-game/dist/assets/images/cloud${$1}.png`' },
    ...
    // 記錄修復後的一些路徑範例
    const afterSamples = jsContent.match(/\/games\/zenbaki-game\/dist\/assets\/[^"]+/g);
    if (afterSamples) {
