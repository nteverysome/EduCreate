...
const filesToFix = [
  ...
  'pages/compare/versions.tsx',
  'pages/preview/version/[id].tsx'
];
...
filesToFix.forEach(filePath => {
  if (fs.existsSync(filePath)) {
    ...
    if (importEndIndex !== -1) {
      content = content.slice(0, importEndIndex) + '\n' + dateUtilsCode + content.slice(importEndIndex);
    } else {
      content = dateUtilsCode + '\n' + content;
    }
...
console.log('\n🎯 所有 date-fns 替換完成！');
console.log('💡 現在可以移除 date-fns 依賴了：npm uninstall date-fns');
