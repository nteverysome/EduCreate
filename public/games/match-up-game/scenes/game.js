...
class GameScene extends Phaser.Scene {
    ...
    saveGameProgressLocally() {
        ...
        try {
            ...
            const gameProgress = {
                ...
                layout: this.layout,
                random: this.random,
                // 追加：保存排序/洗牌快取，避免切換標籤後順序改變
                shuffledPairsCache: Array.isArray(this.shuffledPairsCache) ? this.shuffledPairsCache : null,
                rightCardsOrderCache: Array.isArray(this.rightCardsOrderCache) ? this.rightCardsOrderCache : null,
    ...
    async create() {
        ...
        // 優先從快取復原詞彙，避免切換標籤/最小化後再次顯示「載入詞彙中」
        const width = this.scale.width;
        const height = this.scale.height;

        // 取得 activityId 與快取 key
        const urlParamsForCreate = new URLSearchParams(window.location.search);
        const activityIdForCreate = urlParamsForCreate.get('activityId') || 'unknown';
        const vocabCacheKey = `matchUpVocabularyCache_${activityIdForCreate}`;

        // 內存與 localStorage 快取
        const memoryCache = (window.__matchUpVocabularyCache && window.__matchUpVocabularyCache[activityIdForCreate]) || null;
        let storageCache = null;
        try {
            const raw = localStorage.getItem(vocabCacheKey);
            if (raw) storageCache = JSON.parse(raw);
        } catch (e) {
            console.warn('⚠️ 解析本地詞彙快取失敗，忽略', e);
        }
        ...
        if (memoryCache || storageCache) {
            // 從快取還原
            this.pairs = memoryCache || storageCache;
            success = Array.isArray(this.pairs) && this.pairs.length > 0;
            console.log('✅ 從詞彙快取還原，跳過網路載入', {
                source: memoryCache ? 'memory' : 'localStorage',
                pairsCount: this.pairs?.length || 0
            });
        } else {
            // 無快取時才顯示載入提示並走 API
            console.log('🎮 GameScene: 創建白色背景和載入文字', { width, height });
            this.add.rectangle(width / 2, height / 2, width, height, 0xffffff).setDepth(-1);
            loadingText = this.add.text(width / 2, height / 2, '載入詞彙中...', {
                fontSize: '24px',
                color: '#333333',
                fontFamily: 'Arial'
            }).setOrigin(0.5);

            console.log('🎮 GameScene: 開始載入詞彙數據（無快取）');
            try {
                ...
                // 載入成功後寫入快取（記憶體 + 本地）
                try {
                    window.__matchUpVocabularyCache = window.__matchUpVocabularyCache || {};
                    window.__matchUpVocabularyCache[activityIdForCreate] = this.pairs;
                    localStorage.setItem(vocabCacheKey, JSON.stringify(this.pairs));
                    console.log('💾 已快取詞彙資料到 memory 與 localStorage');
                } catch (cacheErr) {
                    console.warn('⚠️ 詞彙資料快取失敗（可忽略）', cacheErr);
                }
            } catch (error) {
    ...
    updateLayout() {
        ...
        try {
            ...
            //   Restore saved progress once (layout/currentPage/matched pairs/order caches)
            if (!this._progressAppliedOnce) {
                try {
                    const saved = this.loadGameProgressLocally && this.loadGameProgressLocally();
                    if (saved) {
                        if (saved.layout) this.layout = saved.layout;
                        if (typeof saved.currentPage === 'number') this.currentPage = saved.currentPage;
                        if (Array.isArray(saved.matchedPairs)) this.matchedPairs = new Set(saved.matchedPairs);
                        if (saved.timerType) this.timerType = saved.timerType;
                        if (typeof saved.remainingTime === 'number') this.remainingTime = saved.remainingTime;
                        if (Array.isArray(saved.allPagesAnswers)) this.allPagesAnswers = saved.allPagesAnswers;
                        if (Array.isArray(saved.currentPageAnswers)) this.currentPageAnswers = saved.currentPageAnswers;
                        if (Array.isArray(saved.shuffledPairsCache)) this.shuffledPairsCache = saved.shuffledPairsCache;
                        if (Array.isArray(saved.rightCardsOrderCache)) this.rightCardsOrderCache = saved.rightCardsOrderCache;
                        console.log('✅ [restore] 已從本地保存恢復進度與順序快取');
                    }
                } catch (e) {
                    console.warn('⚠️ [restore] 恢復進度失敗（忽略）', e);
                }
                this._progressAppliedOnce = true;
            }
