...
async function collectScreenshots() {
  ...
  try {
    ...
    console.log(`📊 實際生成截圖數量: ${files.length}`);
    console.log('📁 生成的截圖文件:', files);
    ...
    const evidenceReport = {
      ...
      actualFiles: files,
      categories: {
        homepage: files.filter(f => f.includes('homepage')).length,
        dashboard: files.filter(f => f.includes('dashboard')).length,
        functionality: files.filter(f => f.includes('collaboration') || f.includes('tab')).length,
        responsive: files.filter(f => f.includes('responsive')).length
      },
